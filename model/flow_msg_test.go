...
func TestFlowHttp(t *testing.T) {
	...
	flowHttp := &FlowHttp{}
	err := json.Unmarshal([]byte(flowhttp), flowHttp)
	if err != nil {
		t.Fatalf("TestMstHttp failed %s", err)
	}
	t.Logf("%d, %#v", len(flowhttp), *flowHttp)
}
...
func TestFlowHttpSchema1(t *testing.T) {
	...
	documentLoader := gojsonschema.NewStringLoader(flowhttp)
...
func TestFlowHttpSchema2(t *testing.T) {
	...
	var v interface{}
	if err := json.Unmarshal([]byte(flowhttp), &v); err != nil {
		fmt.Printf("The document is not valid. see errors :\n")
		log.Fatal(err)
	}
...
func TestFlowHDnsSchema(t *testing.T) {
	sch, err := jsonschema.Compile("schema/flow_dns_schema.json")
	if err != nil {
		log.Fatalf("%#v", err)
	}

	var v interface{}
	if err := json.Unmarshal([]byte(flowdns), &v); err != nil {
		fmt.Printf("The document is not valid. see errors :\n")
		log.Fatal(err)
	}
