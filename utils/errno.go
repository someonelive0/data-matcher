...
func (p *MyErrors) Push(errlog *ErrorLog) {
	p.mutex.Lock()
	if len(p.Errors) >= p.Capacity {
		half := int(p.Capacity / 2)
		p.Errors = append(p.Errors, p.Errors[:half]...)
		p.Errors = append(p.Errors, errlog)
	} else { // TODO
		p.Errors = append(p.Errors, errlog)
	}
	p.Count = len(p.Errors)
	p.mutex.Unlock()
}
...
func (p *MyErrors) Dump() []byte {
	...
	for i := len(p.Errors); i > 0; i-- {
		...
		b, _ := json.MarshalIndent(p.Errors[i-1], " ", "")
		buf.Write(b)
	}
	p.mutex.Unlock()
...
func NewMyErrorHook() *MyErrorHook {
	return &MyErrorHook{Myerrors: NewMyErrors(200)}
}
